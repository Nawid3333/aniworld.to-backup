#!/usr/bin/env python3
"""
AniWorld.to Anime Scraper and Index Manager
...
from Config.Config3 import EMAIL, PASSWORD, DATA_DIR, SERIES_INDEX_FILE, LOG_FILE
from src.index_manager3 import IndexManager, confirm_and_save_changes, show_vanished_series, _extract_slug_from_field, get_episode_counts
from src.Scraper3 import AniWorldScraper, MAX_WORKERS

# Pre-compiled regex for URL validation
_ANIME_URL_RE = re.compile(r'/anime/stream/[^/]+')
...
def print_header():
    print("\n" + "="*60)
    print("  ANIWORLD.TO ANIME SCRAPER & INDEX MANAGER")
    print("="*60 + "\n")
...
def validate_credentials():
    ...
    if not EMAIL or not PASSWORD:
        ...
        print("2. Add your aniworld.to email and password to the .env file")
...
def scrape_all_series():
    ...
    print("\n→ Starting AniWorld.to complete scraper...")
    print("  (Browser will open - do not close it manually)\n")
...
def add_single_series(url):
    ...
    if not _ANIME_URL_RE.search(urlparse(url).path):
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line_num, line in enumerate(f, 1):
                ...
                if parsed.scheme and parsed.scheme not in ('http', 'https'):
                    ...
                    continue
                if not _ANIME_URL_RE.search(parsed.path):
                    skipped.append((line_num, url))
                    continue
                urls.append(url)
        if skipped:
...
def _show_ongoing_and_export(report, index_manager):
    ...
    export = input(f"\nExport {ongoing_count} ongoing anime URLs to series_urls.txt? (y/n): ").strip().lower()
    if export == 'y':
        try:
            urls = []
...
def _print_report_summary(report, report_file, filter_name=None):
    ...
    print(f"  Avg episodes/anime:  {stats.get('average_episodes_per_series', 0)}")
...
def scrape_subscribed_watchlist():
    """Scrape subscribed and watchlist anime from account pages"""
    ...
    try:
        ...
        if not account_series:
            print("\n⚠ No anime found on your account pages.")
            return
        ...
        if resume and scraper.load_checkpoint():
            ...
            if not remaining:
                print("✓ All anime already scraped")
                scraper.clear_checkpoint()
                return
            account_series = remaining

        print("→ Using parallel mode for account anime...")
        ...
        if injected_count_wl > 0:
            print(f"\n  ⚠ {injected_count_wl} anime no longer on watchlist (will prompt for confirmation):")
            for name in injected_names_wl:
                print(f"    • {name}")
        if injected_count_sub > 0:
            print(f"\n  ⚠ {injected_count_sub} anime no longer subscribed (will prompt for confirmation):")
        ...
        if scraper.series_data:
            index_manager = IndexManager(SERIES_INDEX_FILE)
            if confirm_and_save_changes(scraper.series_data, "Account anime", index_manager):
                print("\n✓ Account anime scraping completed!")
                print_completed_series_alerts(index_manager)
                logger.info("Account anime scraping completed")
        else:
            print("\n⚠ No data scraped")
        ...
        if scraper and scraper.series_data:
            ...
            if confirm_and_save_changes(scraper.series_data, "Account anime", index_manager):
                print(f"\n✓ Partial data saved ({len(scraper.series_data)} series)")
                logger.info("Account anime interrupted — partial data saved")
